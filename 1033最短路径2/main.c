...
int v[MAXN], d[MAXN], w[MAXN][MAXN], cnt[MAXN], num[MAXN][MAXN];   //w存路径的权值， num数组表示该路径的条数
int n, m;                                       //cnt存有几条最短路径， d存最小距离
...
void Dijkstra(int v0) {                         //v0表示起始结点
    ...
    for (int i = 0; i < n; i++) {
        ...
        cnt[i] = (i == v0 ? 1 : 0);             //计算有几条最短路径，若没有，就置为0
    }
    for (int i = 0; i < n; i++) {              //从0遍历到n-1，以此求出到n个结点的最小路径，但并不是依次求的
        ...
        for (int y = 0; y < n; y++)    //接下来更新所有标记为0的结点到v0的最小距离，比较d[y]和d[x]+w[x][y]
            if (!v[y]) {
                if (d[x] + w[x][y] == d[y])                     //如果这两种路径距离相等
                    cnt[y] += cnt[x] * num[x][y];               //更新最小路径的条数
                else
                    if (d[x] + w[x][y] < d[y]) {                //如果找到更短的路径
                        cnt[y] = cnt[x] * num[x][y];            //更新最短路径的条数
